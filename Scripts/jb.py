...
model.reactions.get_by_id('BIOMASS_aerobic').lower_bound = 0.287
model.reactions.get_by_id('BIOMASS_aerobic').upper_bound = 1000
#model.reactions.get_by_id('PGI').lower_bound = 0
#model.reactions.get_by_id('PGI').upper_bound = 1000
'''with model:
    model.genes.128_13.knock_out()'''
...
en =np.array(production_envelope  (model,['EX_o2_e'],objective= 'BIOMASS_aerobic', carbon_sources= 'EX_glc__D_e',points = 1000))
np.savetxt('BIOMASS_aerobic.csv',en,fmt='%s')
print (en)
